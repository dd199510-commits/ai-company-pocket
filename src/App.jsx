...
const OFFICE_STAGE_WIDTH = 1200
const OFFICE_STAGE_HEIGHT = 720
...
function AgentFigure({ agent }) {
  return (
    <span
      className={[
        'agent-figure',
        agent.supervisor ? 'agent-figure-supervisor' : '',
        `agent-figure-${agent.state}`,
      ].filter(Boolean).join(' ')}
      style={{
    ...
    >
      <span className="agent-figure-shadow" aria-hidden="true" />
      <span className="agent-figure-head" aria-hidden="true" />
      <span className="agent-figure-body" aria-hidden="true">
        <span className="agent-figure-label">{agent.label}</span>
      </span>
    </span>
  )
}
...
function AgentStation({ agent, selected, onSelect }) {
  const style = {
  ...
  return (
    ...
    >
      <span className="agent-station-art">
        <span className="agent-desk" aria-hidden="true">
          <span className="agent-desk-top" />
          <span className="agent-desk-leg agent-desk-leg-left" />
          <span className="agent-desk-leg agent-desk-leg-right" />
          <span className="agent-desk-line agent-desk-line-strong" />
          <span className="agent-desk-line agent-desk-line-soft" />
          <span className="agent-desk-widget" />
          <span className="agent-desk-tab" />
          <span className="agent-desk-task">{agent.metric}</span>
        </span>
        <span className="agent-station-person">
        ...
        <span className="agent-work-bubble">{agent.bubble}</span>
...
function agentLinkPoint(agent, role = 'station') {
  const x = Number.parseFloat(agent.layout.x)
  const y = Number.parseFloat(agent.layout.y)
  if (agent.id === 'main') return { x, y: y + 9 }
  return { x, y: role === 'target' ? y - 8 : y - 2 }
}

function buildRoutePath(source, target) {
  const from = agentLinkPoint(source, 'source')
  const to = agentLinkPoint(target, 'target')
  const midY = (from.y + to.y) / 2
  return `M ${from.x} ${from.y} C ${from.x} ${midY}, ${to.x} ${midY}, ${to.x} ${to.y}`
}

function AgentLinks({ agents, activeTask }) {
  const main = agents.find((agent) => agent.id === 'main')
  if (!main) return null
  const mainX = Number.parseFloat(main.layout.x)
  const mainY = Number.parseFloat(main.layout.y)
  ...
  const routePath = routeSource && routeTarget && routeSource.id !== routeTarget.id
    ? buildRoutePath(routeSource, routeTarget)
    : ''
  return (
    <svg className="agent-link-layer" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
      <defs>
        <marker
        ...
        >
          <path d="M 0 0 L 5 2.5 L 0 5 z" className="agent-route-arrow" />
        </marker>
      </defs>
      {agents.filter((agent) => agent.id !== 'main').map((agent) => {
        ...
        return (
          ...
          >
            <path
              d={`M ${mainX} ${mainY + 9} C ${mainX} ${(mainY + y) / 2}, ${x} ${(mainY + y) / 2}, ${x} ${y - 8}`}
            />
            {active ? <circle cx={x} cy={y - 8} r="1.2" /> : null}
          </g>
        )
      })}
      {routePath ? (
        <g className="agent-route">
          <path className="agent-route-path" d={routePath} markerEnd="url(#agent-route-arrow)" />
          <circle r="1.05" className="agent-route-pulse">
...
function WorkspaceStatusBoard({ agents, activeTask, pendingAction }) {
...
function AgentWorkspace({ agents, selectedId, onSelect, activeTask, pendingAction }) {
